...
def pcd_to_np(pcd_file,t=None):
    ...
    if t is not None:
        intensity = xyz[:, 0] * 0 + t
        pcd_np = np.hstack((xyz, intensity.reshape(-1,1)))
    else:
        pcd_np = xyz
...
def mask_points_by_range(points, limit_range):
    ...
    if isinstance(points,list):
        for i, point in enumerate(points):
            mask = (point[:, 0] > limit_range[0]) & (point[:, 0] < limit_range[3])\
            & (point[:, 1] > limit_range[1]) & (
                    point[:, 1] < limit_range[4]) \
            & (point[:, 2] > limit_range[2]) & (
                    point[:, 2] < limit_range[5])
            points[i] = point[mask]
...
def mask_ego_points(points_list):
    """
    ...
    if isinstance(points_list,list):
        ...
        points = points_list
        if points.shape[0]>=1 and points.shape[1]>=1:
            mask = (points[:, 0] >= -1.95) & (points[:, 0] <= 2.95) \
                & (points[:, 1] >= -1.1) & (points[:, 1] <= 1.1)
            points = points[np.logical_not(mask)]
