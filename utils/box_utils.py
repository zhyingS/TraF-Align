...
from utils.transformation_utils import x1_to_x2
...
def project_box7d(box7d, transformation_matrix):

    assert transformation_matrix.shape == (4, 4)
    torch_ = False
    if isinstance(box7d,torch.Tensor):
        box7d=np.array(box7d.cpu())
        torch_ = True

    if isinstance(transformation_matrix, torch.Tensor):
        transformation_matrix=np.array(transformation_matrix.cpu())

    R,t=transformation_matrix[:-1,:-1],transformation_matrix[:-1,-1]
    box7d[:,0:3]=(np.dot(R,box7d[:,0:3].T)+t.reshape(-1,1)).T
    heading=np.hstack((np.cos(box7d[:,-1]).reshape(-1,1),np.sin(box7d[:,-1]).reshape(-1,1)))
    c=Rotation.from_matrix(R).as_euler('xyz')
    box7d[:,-1]=c[-1]+box7d[:,-1]

    if torch_:
        return torch.tensor(box7d)
    else:
        return box7d
