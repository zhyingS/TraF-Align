...
def x1_to_x2_matrix(x1_to_world, x2_to_world, correction = np.zeros((3,))):
    """
    Transformation matrix from x1 to x2.

    Parameters
    ----------
    x1_to_world : np.array, 4x4
        x1 to world
    x2_to_world : np.array, 4x4
        x2 to world
    ...
    transformation_matrix[:-1,-1] = transformation_matrix[:-1,-1]+ correction.reshape(-1,)
