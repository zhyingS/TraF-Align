...
from models.modules.compressor.naive_compress import NaiveCompressor
from models.modules.utils.downsample_conv import DownsampleConv
from models.modules.fusion.v2v_fuse import V2VNetFusion
...
class V2VNet(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg["model"]["compression"] != 0:
            self.compression = True
            self.naive_compressor = NaiveCompressor(256, cfg)
        else:
            self.compression = False

        self.fusion_net = V2VNetFusion(cfg["model"]["v2vnet_fusion"])
