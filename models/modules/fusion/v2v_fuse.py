...
from models.utils.torch_transformation_utils import \
    get_discretized_transformation_matrix, get_transformation_matrix, \
    warp_affine, get_rotated_roi
from models.utils.convgru import ConvGRU
        ...
        for l in range(self.num_iteration):
            ...
            for b in range(B):
                ...
                if N > 0:
                    ...
                    batch_updated_node_features.append(
                     torch.zeros((0,C,H,W)).type_as(x))
            batch_node_features = batch_updated_node_features
        # (B,C,H,W)
        li = []
        for itm in batch_node_features:
            if itm.shape[0]>0:
                li.append(itm[0, ...].unsqueeze(0))
            else:
                li.append(torch.zeros((1,C,H,W)).type_as(x))
        out = torch.cat(li,dim=0)
        # out = torch.cat(
